...
class WebApiActor(
   ...
   */
  implicit override def actorRefFactory: ActorSystem = system

  /**
   * this actor only runs our route, but you could add other things here,
   * like request stream processing or timeout handling
   */
  def receive: Receive = runRoute(myRoutes ~ commonRoutes)
}

/**
 *
 */
abstract class WebApi(system: ActorSystem,
             config: Config,
             port: Int,
             jarManager: ActorRef,
             dataManager: ActorRef,
  ...
  implicit val ShortTimeout = Timeout(config.getDuration("spark.jobserver.short-timeout",
