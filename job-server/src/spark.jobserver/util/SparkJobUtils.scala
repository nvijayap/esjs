...
//              TimeUnit.MILLISECONDS).toInt / 1000).getOrElse(40)
        Try(config.getInt("spark.jobserver.yarn-context-creation-timeout") / 1000).getOrElse(40)
      case _               =>
//        Try(config.getDuration("spark.jobserver.context-creation-timeout",
//              TimeUnit.MILLISECONDS).toInt / 1000).getOrElse(15)
          Try(config.getInt("spark.jobserver.context-creation-timeout") / 1000).getOrElse(15)
    }
  }
}
